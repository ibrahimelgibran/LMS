		...
		<div class="col-md-8">
			<div class="row">
				<div class="col-md-3">
					...
					<?php if ($paypal[0]->active != 0) : ?>
						<div class="row payment-gateway paypal" onclick="selectedPaymentGateway('paypal')">
							<div class="col-12">
								<img class="tick-icon paypal-icon" src="<?php echo base_url('assets/payment/bsi.png'); ?>">
								<img class="payment-gateway-icon" src="<?php echo base_url('assets/payment/bsi.png'); ?>">
							</div>
						</div>
					<?php endif;
					if ($stripe[0]->active != 0) : ?>
						<div class="row payment-gateway stripe" onclick="selectedPaymentGateway('stripe')">
							<div class="col-12">
								<img class="tick-icon stripe-icon" src="<?php echo base_url('assets/payment/dana.png'); ?>">
								<img class="payment-gateway-icon" src="<?php echo base_url('assets/payment/dana.png'); ?>">
							</div>
						</div>
					<?php endif; ?>
				...
				<div class="col-md-8">
					...
					<a href="http://ibrahimelgibran.com">
						<form action="<?php echo site_url('home/paypal_checkout'); ?>" method="post" class="paypal-form form">
							...
							<button type="submit" class="payment-button float-right"><?php echo get_phrase('pay_by_bsi'); ?></button>
						</form>
						</a>
						 -->

					<div class="w-100 float-left">
						<form action="<?php echo site_url('home/paypal_checkout'); ?>" method="post" class="paypal-form form" id="payment-form">
							<hr class="border mb-4">
							<input type="hidden" name="total_price_of_checking_out" value="<?php echo $total_price_of_checking_out; ?>">
							<!-- Ganti tombol menjadi anchor (link) -->
							<a href="javascript:void(0)" onclick="submitForm()" class="payment-button float-right"><?php echo get_phrase('pay_by_bsi'); ?></a>	
						</form>	
					</div>

					<script>
						function submitForm() {
							var phoneNumber = '6285876076005'; // Ganti dengan nomor WhatsApp Anda
							var message = encodeURIComponent("Hi There, Salam Kenal! \n\nTerima kasih telah mengikuti course untuk belajar bersama dengan IEGPEDIA.\n\nSaya tertarik dengan paket kursus tersebut dan silahkan lakukan pembayaran berikut sesuai harga course tersebut.\n_____________\n\nSesuaikan bioodata anda pada IEGPEDIA!\nUsername:\nEmail\nPaket Course:\nHarga: Rp\n_____________\n\na.n IBRAHIM EL GIBRAN\nBSI\n7205751291\n_____________\n\nKirim screenshot Bukti TF dan kalau membutuhkan bantuan terkait teknis. Silahkan hubungi admin atau chat live di fitur message yang sudah ada pada instruktur.\n\nThanks again for choosing IEGPEDIA!");

							// Membuat tautan WhatsApp dengan nomor dan pesan pra-isi
